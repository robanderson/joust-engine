...
- **Run-depth override** (steelman budget): a marker-adjacent `fast` / `deep` (same adjacency + strip rules as the size words; e.g. `@@JE:5 fast, <task>`, `deep @@JE:5`) sets how many steelman boost-and-cold-re-judge iterations a FINAL decision point may run before a persistent tie routes to `needs_orchestrator_pick`: `fast` = 1 (the one mandatory shootout round only), `deep` = up to 5, absent = the engine default (3). The parser emits `depth` (`fast`|`deep`|`null`) and, when set, `steelmanMaxIters` (1|5) — pass `steelmanMaxIters` straight through to the Phase 2 workflow args. An ordinary `fast`/`deep` in the task body is never misread.
- **Plan/Implement round split.** The tournament is a wide **Plan phase** (Plan Round 1 + Plan Round 2, always — attempts produce **DESIGN BRIEFS** (<=10 bullets: approach/surfaces/risks/testable acceptance criteria; hard altitude rule — no code, diffs, or line numbers, violations judged DOWN) judged by the plan-lens council) plus an optional narrow **Implement phase** (Implement Round 3, plus Round 4 only if R3 has no gate-passing candidate — implementers seeded with the winning brief as an approach + acceptance-criteria contract, judged by the code-lens council). The parser exposes:
...
6. **`implement: true` → run the Implement phase after the Plan phase.** Pass `implement: true` and `implementAttempts` (from `implementAssignment`, or the implement default pool) to the Phase 2 workflow. The engine runs the plan phase first (always), and only on a **resolved winning design brief** proceeds to Implement Round 3 (+ Round 4 if R3 has no gate-passing candidate). If the plan phase returns `no_consensus`, surface the split and stop — implementation never runs without a consensus brief. If Round 4 also fails the gate, the result carries `needs_human: true` (existing contract).
...
Branch naming:  JE-<loop>-<random7>   (NOTE: this fixed name is used as-is, OVERRIDING any branch-prefix rule you have configured, for these loop branches only)
...
> 8. Codex — OpenAI gpt-5.5 / gpt-5.6 via the `codex exec` CLI (I'll then ask which model / effort)
...
- **Option 8 (Codex):** Codex serves **two model families**: **gpt-5.5**, where the submenu axis is the **reasoning effort** (codex's quality lever on 5.5), and the **gpt-5.6 family** (base `gpt-5.6` plus the named variants `codex-sol`/`codex-terra`/`codex-luna`, each pinned to effort high; a bare `codex` in a prose spec means `codex-sol`). First run a one-line liveness probe so a stale CLI / auth block doesn't waste a (possibly paid) round:
  ...
  > Which codex model?
  > 1. codex-sol — gpt-5.6-sol, effort high (the bare-`codex` default)
  > 2. codex-terra — gpt-5.6-terra, effort high
  > 3. codex-luna — gpt-5.6-luna, effort high
  > 4. gpt-5.6 — the 5.6 base model, effort high
  > 5. gpt-5.5 Low — fastest, lightest reasoning
  > 6. gpt-5.5 Medium — balanced
  > 7. gpt-5.5 High — deeper reasoning
  > 8. gpt-5.5 Extra high — maximum reasoning depth

  Record the uniform assignment using the displayModel (`codex-sol`/`codex-terra`/`codex-luna`/`gpt-5.6`, or `codex-<effort>` for the gpt-5.5 tiers — effort token: low / medium / high / **xhigh** for "Extra high"), e.g. for N = 4 at 5.5 High: `[codex-high, codex-high, codex-high, codex-high]`. Codex bills your OpenAI/ChatGPT plan, not Anthropic usage; flag that codex (an autonomous agent with no turn cap) is slower than a one-shot, so size N modestly.
...
It prints the full guard set as JSON: `attemptMaxTurns` (GLM), `localMaxTurns`, `minimaxMaxTurns`, `grokMaxTurns`, `attemptTimeoutSecs` (local/base), `glmTimeoutSecs`, `codexTimeoutSecs`, `grokTimeoutSecs`, `minimaxTimeoutSecs`. Pass **every** field through to the Phase 2 workflow args verbatim — each key is exactly the arg name the engine reads, so the values flow into the runners as `JE_MAX_TURNS` / `JE_TIMEOUT_SECS`. Roughly: short tightens the guards (e.g. GLM 15 turns / 180s, codex 300s), medium matches the historical defaults (GLM 30 turns / 300s, codex 600s, with a roomier 1200s GLM wall-clock since z.ai is slow, and a 900s MiniMax wall-clock — M3 blew the shared 300s on real medium tasks, issue #30), long loosens them for big work (GLM 50 turns / 600s, GLM wall-clock 2400s, codex/grok 1200s, MiniMax 1800s).
...
- A cost note scaled to the mode. **Each judging point is a lens council voting ONCE** (judging-v3: no deliberation — an intermediate review is one parallel vote round; a final decision point is one vote round + the steelman shootout: 1 steelman + ≤2 boost implementers + a two-candidate cold re-vote per iteration, iterating only while tied, max 5), so a "judging point" is ~5–15 calls:
  ...
  - **Mixed-family seats (default):** by default 2 of the 5 seats in each council (spec/craft for code, completeness/simplicity for plan) run on **codex-xhigh** instead of Opus — this adds codex spend/latency to every judging point (on top of any attempt-level codex usage) and, since codex has no turn cap, its wall-clock timeout (`codexJudgeTimeoutSecs`, default 1500s) can be the long pole for a round. Pass `judgeMix: 'anthropic'` to keep judging all-Opus (byte-identical to pre-mixed-family behaviour) if the codex judging cost/latency isn't wanted. The codex-xhigh judge seats reuse the existing `joust-codex` agent + `codexRunner`, so pass `codexRunner` even for a judge-only-mixed run.
...
- **Preferred — dynamic workflows.** Invoke the bundled Workflow script `workflows/tournament.mjs` (at the plugin root, i.e. `../../workflows/tournament.mjs` relative to this skill's base dir) via the `Workflow` tool, passing `args` (see `references/orchestration.md` for the exact shape: task, mode, runDir, optionally `workspaceRoot` (self-contained candidate workspaces default to `/tmp/je-workspaces/<run-id>`, outside the plugin cache, so runner-based attempts — glm, minimax, codex, grok — can actually save their deliverable; pass `workspaceRoot: runDir` for the pre-#34 layout), the per-attempt list, and the runner paths each kind of attempt needs — plus, if the task has **known input files every worker needs** (e.g. "evaluate/summarise/audit these files"), pass them as `contextFiles: [paths]` so the engine bundles them once and every attempt reads the bundle instead of re-reading each file (this avoids the dominant duplicated-Read cost) — `glmRunner` = `<plugin-root>/bin/glm-run.sh` if any attempt is GLM, `localRunner` = `<plugin-root>/bin/local-run.sh` if any attempt is Local, `codexRunner` = `<plugin-root>/bin/codex-run.sh` if any attempt is Codex, pass `depth` and `steelmanMaxIters` when Phase 0 returned them (the `fast`/`deep` run-depth keyword: fast/default implement runs SKIP the Round-2 plan rewrite — the steelman iterations on the winning brief replace it; only `deep` keeps Round 2; an explicit `:2` sigil is always honoured; omitted steelmanMaxIters = engine default 3), and the **per-attempt limit args come from the Phase 1c size profile** (`node <plugin-root>/bin/je-parse.mjs --size <short|medium|long>`) — pass every field it prints through verbatim: `attemptMaxTurns` = the agentic-iteration cap for **GLM** runners, `localMaxTurns` / `minimaxMaxTurns` / `grokMaxTurns` = the caps for the other runner providers (the iteration backstop against grinding; the hard-stop brief is the real guard), `attemptTimeoutSecs` = the wall-clock backstop for local/base, `glmTimeoutSecs` = the GLM-only wall-clock (z.ai is slow on heavy code), `minimaxTimeoutSecs` = the MiniMax-only wall-clock (M3 is slow on real code — issue #30), `codexTimeoutSecs` = the Codex-only wall-clock (codex `exec` has **no** turn cap, so the wall clock is its only per-attempt guard), `grokTimeoutSecs` = the Grok-only wall-clock. Also pass **`issueRunner`** = `<plugin-root>/bin/je-issue.sh` so the engine can auto-file engine-fault RC classes as privacy-scrubbed `dogfood` issues at run end (fire-and-forget; omit it to skip auto-filing, or pass `noAutoIssue: true` if the user opts out — RCs are still derived and reported either way). If the user gave no size and you did not estimate one, the engine still applies its own built-in medium-ish defaults, but you should always pass an explicit profile so the limits match the task. The workflow runs Phases 2–5 deterministically — parallel attempts, the blind Opus review, and (two pass) round two and the final rank — and you can watch it live in `/workflows`. It returns the structured mapping + rankings you report in Phase 6. This is opt-in orchestration: the harness will show a confirm at first dispatch. **Anthropic attempts run native; GLM attempts run through `joust-glm-*` agents executing `bin/glm-run.sh` (z.ai); Local attempts run through the single `joust-local` agent executing `bin/local-run.sh` (on-device omlx server); Codex attempts run through the single `joust-codex` agent executing `bin/codex-run.sh` (OpenAI `codex exec`).** The Task tool cannot target GLM, local, or codex models directly, and the runner-script indirection is what makes those paths reliable (do not inline the raw nested-`claude`/`codex` command; the wrapper will refuse or shortcut it). The indirection matters most for codex, a fully autonomous external agent most prone to solving/refusing the task itself.
...
- **Isolation.** Each attempt gets its own workspace (e.g. `round-1/candidate-<i>/` under the workspace root — `/tmp/je-workspaces/<run-id>` by default for self-contained mode, `<run-id>/round-1/candidate-<i>/` under `worktreeRoot` in repo-anchored mode; see "Run layout" in `references/orchestration.md`). Isolated workspaces prevent the race conditions and clobbered files that parallel writes to one location cause.
...
**Run-repo results bus (optional, tracker #21).** Passing `runRepoBase` in the workflow args (a push-to-create remote base, e.g. `git@<your-git-host>:<org>` — always a **generic placeholder** in anything committed; the real host lives only in machine-local configuration) turns on a per-run git mirror: the engine creates `je-run-<runId>` via push-to-create, each attempt's finish pushes its deliverables to a per-worker orphan branch under a **neutral identity** (`worker-<label> <worker@je>` — no model identity reaches the bus mid-run) plus its runner log to main, and the identity-revealing artifacts (mapping, summaries, council verdicts) are published to main **only after the run's terminal persist** (`bin/je-run-repo.sh` documents the sequencing rule). The whole bus is strictly **fail-open**: any push failure logs `JE-RUNREPO-FAIL <op>` and the run continues — the local `.runs` artifacts remain the source of truth. Requirements: `JE_RUN_REMOTE_BASE`/`runRepoBase` set (absent ⇒ the helper exits with the distinct "run-repo disabled" code and the feature is simply off), and the git host reachable by the helper agents — which means the operator's **permission allowlist must cover `git push`/`git clone` to that host**. Put both the real remote base and those allowlist entries in **machine-local settings (`settings.local.json`)**, never in committed project settings — this is a public repo and no LAN hostname may land in it.
...
Build the final pool: the **N fresh round two attempts plus the carried round-one champion(s)** — one on a round-1 majority, TWO on a fast-tally split, none if all were vetoed — re-labelled blind. The **same council** seeds the top-2 finalists with one vote round and resolves them through the **steelman shootout** (always ≥1 improvement round; cold re-judges; max 5; then `needs_orchestrator_pick` — see Phase 3 and `references/review-rubric.md`). Carried champions compete blind on the merits like everything else. The winner ships with its steelman improvements applied. (`judges: 1` uses a single blind Opus ranker instead.)
...
5. **Surface `rc_summary.non00`.** The return value carries `rc_summary { seats, by_code, non00: [{seat, phase, rc, reason}] }` (also rendered as a table in `SUMMARY.md`). List **every** non-00 seat — its seat id, phase, RC class, and reason — so there is no more silent seat loss. An all-00 run needs only a one-line "all seats returned 00" note. Engine-fault classes (RC 01/02 after retries, 04–09 — not honest model losses or the 03 turn-cap) are **auto-filed** to the engine repo as one deduped, privacy-scrubbed `dogfood` issue per class per run (fail-closed; never blocks the run) unless `noAutoIssue: true` was passed.
...
4. **Run the tournament (UNCHANGED engine) via the Workflow tool.** Invoke `workflows/tournament.mjs` exactly as in Phase 2, with `runDir: "<runDir>/loop-<k>"` and the task **augmented with the cross-loop ledger** (see below). It returns the structured mapping/ranking; **pick the winning candidate's deliverable path** (its proposal artifact) from `final.mapping`/`round1.mapping`. The proposal is a design brief — approach + acceptance criteria; implementation details belong to the implementer (Phase 2 already briefs attempts to produce that).
...
- Phase 1 model question: ten options (Top Mixed, Specify Mix, Opus, Sonnet, Haiku, GLM→submenu, Local→live submenu, Codex→effort submenu, MiniMax, Grok→variant submenu); a Phase-0 prose spec or Top Mixed keyword answers it and the menu is skipped; N defaults 6, passes default 2. GLM drills down to one of glm-5.2/glm-5.1/glm-4.7/glm-4.5-air; Local lists `omlx-models` live (dynamic); Codex has a model submenu spanning the gpt-5.6 family (codex-sol/terra/luna, gpt-5.6 — effort pinned high; bare `codex` = codex-sol) and the gpt-5.5 effort tiers (codex-low/medium/high/xhigh); Grok drills down to grok-build/grok-composer-2.5-fast; Specify Mix loops per attempt over Anthropic + GLM + local + codex + minimax-m3 + grok ids; in two pass the assignment applies to both rounds. Anthropic attempts dispatch via the Task tool, GLM via the `glm`→z.ai runner, Local via the `omlx`→on-device runner, Codex via the `codex exec` runner, MiniMax via the `bin/minimax-run.sh` runner, Grok via the `grok`→xAI runner. Reviewer/ranker are always Anthropic Opus (a 5-lens council by default; `judges: 1` = single blind Opus judge).
...
- Dynamic limits (Phase 1c): per-attempt turn caps + wall-clock timeouts scale to task size (`short`/`medium`/`long`). The parser's `size` field is a manual override (marker-adjacent `@@JE:5 long`); otherwise the orchestrator estimates. Resolve the numbers with `bin/je-parse.mjs --size <label>` (one source of truth: `SIZE_PROFILES`) and pass them into the Phase 2 workflow args (`attemptMaxTurns`, `localMaxTurns`, `minimaxMaxTurns`, `grokMaxTurns`, `attemptTimeoutSecs`, `glmTimeoutSecs`, `codexTimeoutSecs`, `grokTimeoutSecs`, `minimaxTimeoutSecs`). They flow to the runners as `JE_MAX_TURNS`/`JE_TIMEOUT_SECS`; native Anthropic attempts are uncapped.
- Round attempts: N parallel, isolated, identical brief plus one diversity modifier, no cross-talk.
- Review/rank (Opus, blind) is a **lens council** by default: judges vote independently ONCE, a **>50% majority** is tallied in **code** (never an LLM), and the **security lenses can veto** with evidence. Judging-v3: NO deliberation — intermediate reviews fast-tally (split carries the top-2 non-vetoed into the final pool); final decision points run the steelman shootout (seed top-2 → judge-guided boosts → cold re-judge, max 5 rounds → `needs_orchestrator_pick`); **NO_CONSENSUS** remains only for all-vetoed (surface interactively / grand-loop needs-human + HALT; never a synthesised winner). Two-pass guidance is a separate synthesis step (not a decision-maker). Per-judge verdicts, per-round tally, and veto events are persisted (`review-*/council.json`, `verdict.md`, summaries). `judges: 1` = the legacy single blind Opus judge (byte-for-byte prior behaviour); council size is otherwise fixed at 5. By default codex-xhigh judges the spec/craft seats (code) and completeness/simplicity seats (plan) via the `joust-codex` agent + `codexRunner` (`judgeMix: 'anthropic'` = all-Opus, byte-for-byte; a codex seat that fails twice falls back to Opus); every judge's brief is pinned to the tournament snapshot and forbidden from the live checkout.

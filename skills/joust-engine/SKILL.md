...
- **Prose marker** `joust engine:N[:M[:Z]]` — extended identically.
...
|------|-------------|----------|
| Trigger | `@@JE[:N][:M[:Z]]` / `joust engine:N[:M[:Z]]` — N optional (prose spec / Top Mixed can supply it) | same, `:2` = two pass |

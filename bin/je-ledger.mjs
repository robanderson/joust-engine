...
const SEAT_OPTIONAL = ['round', 'failReason', 'rc', 'mechanical', 'seedBrief']
...
export function aggregate(records) {
  ...
  for (const rec of records) {
    ...
    const im = seatModel(rec, ['implement'], rec.implementWinner)
    if (im) stat(im).implWins++
...
function main(argv) {
  ...
  console.error('usage: je-ledger.mjs <record <runDir> | report>    (ledger: $JE_LEDGER_PATH or ~/.joust-engine/ledger.jsonl)')
  return 2
}
